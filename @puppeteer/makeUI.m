...
for i = 1:n_controls
    ...
    controllabel(i) =  uilabel(self.handles.fig,'Position',[80 height-i*slider_spacing+20 230 20],'FontSize',14,'Text',thisstring,'BackgroundColor','w','HorizontalAlignment','center');


    self.handles.lbcontrol(i) = uieditfield(self.handles.fig,'numeric','Position',[20 height-i*slider_spacing-7 40 20],'Value',lb(i),'ValueChangedFcn',@self.resetSliderBounds,'Tag',mat2str(i));
    self.handles.ubcontrol(i) = uieditfield(self.handles.fig,'numeric', 'Position',[330 height-i*slider_spacing-7 40 20],'Value',ub(i),'ValueChangedFcn',@self.resetSliderBounds,'Tag',mat2str(i),'HorizontalAlignment','left');
